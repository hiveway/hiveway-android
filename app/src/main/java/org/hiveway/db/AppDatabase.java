...
@Database(entities = {PostEntity.class, AccountEntity.class}, version = 5, exportSchema = false)
public abstract class AppDatabase extends RoomDatabase {
    ...
    public static final Migration MIGRATION_2_3 = new Migration(2, 3) {
        ...
        public void migrate(@NonNull SupportSQLiteDatabase database) {
            database.execSQL("CREATE TABLE PostEntity2 (uid INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, text TEXT, urls TEXT, contentWarning TEXT);");
            database.execSQL("INSERT INTO PostEntity2 SELECT * FROM PostEntity;");
            database.execSQL("DROP TABLE PostEntity;");
            database.execSQL("ALTER TABLE PostEntity2 RENAME TO PostEntity;");
    ...
    public static final Migration MIGRATION_3_4 = new Migration(3, 4) {
        @Override
        public void migrate(@NonNull SupportSQLiteDatabase database) {
            database.execSQL("ALTER TABLE PostEntity ADD COLUMN inReplyToId TEXT");
            database.execSQL("ALTER TABLE PostEntity ADD COLUMN inReplyToText TEXT");
            database.execSQL("ALTER TABLE PostEntity ADD COLUMN inReplyToUsername TEXT");
            database.execSQL("ALTER TABLE PostEntity ADD COLUMN visibility INTEGER");
        }
    };
